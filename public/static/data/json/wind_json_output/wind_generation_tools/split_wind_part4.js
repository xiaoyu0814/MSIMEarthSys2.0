...
// 读取原始数据
const inputPath = path.join(__dirname, 'wind_part4.json');
...
for (let yIdx = 0; yIdx < numY; yIdx++) {
  ...
  for (let xIdx = 0; xIdx < numX; xIdx++) {
    ...
    // 保存文件
    const outputPath = path.join(__dirname, 'wind_part4_subparts', `wind_part4_${yIdx * 3 + xIdx + 1}.json`);
    const outputData = {
      header: newHeader,
      data: {
        u: newU,
    ...
    console.log(`  保存到: wind_part4_subparts/wind_part4_${yIdx * 3 + xIdx + 1}.json`);
    console.log(`    (${newLo1.toFixed(6)}-${newLo2.toFixed(6)} E, ${newLa1.toFixed(6)}-${newLa2.toFixed(6)} N)`);
    console.log();
...
console.log('\n✓ 完成！9个文件已生成到 wind_part4_subparts/ 目录');
