...
function generateGrid(params) {
  ...
  try {
    // 计算边界范围
    const minLon = fovPointData.length > 0 ? fovPointData.reduce((min, p) => p[0] < min ? p[0] : min, fovPointData[0][0]) : 0;
    const maxLon = fovPointData.length > 0 ? fovPointData.reduce((max, p) => p[0] > max ? p[0] : max, fovPointData[0][0]) : 0;
    const minLat = fovPointData.length > 0 ? fovPointData.reduce((min, p) => p[1] < min ? p[1] : min, fovPointData[0][1]) : 0;
    const maxLat = fovPointData.length > 0 ? fovPointData.reduce((max, p) => p[1] > max ? p[1] : max, fovPointData[0][1]) : 0;
    ...
    const heights = interpolatedData.map((item) => 1260.99 - parseFloat(item.value));
    const minHeight = heights.length > 0 ? heights.reduce((min, val) => val < min ? val : min, heights[0]) : 0;
    const maxHeight = heights.length > 0 ? heights.reduce((max, val) => val > max ? val : max, heights[0]) : 0;
