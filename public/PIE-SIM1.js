(function (global, factory) {
	...
	function DownloadJSON(data, filename) {
	    ...
	    dlLink.download = filename;
	    dlLink.href = urlObject.createObjectURL(blob);
	    dlLink.dataset.downloadurl = ['application/json', dlLink.download, dlLink.href].join(':');
	 ...
	 * @Date: 2024-01-29 17:54:16
	 * @LastEditors: 谢小宇 xiexiaoyu@piesat.cn
	 * @LastEditTime: 2024-05-23 17:59:24
	 * @FilePath: \sim-web\src\SimDataModel\NodeConfig.js
	 ...
	 * @Date: 2024-01-29 17:54:16
	 * @LastEditors: 谢小宇 
	 * @LastEditTime: 2024-05-31 15:29:01
	 * @FilePath: \sim-web\src\SimDataModel\NavPath.js
	 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
	 */
	...
	NavPath.prototype = Object.assign({
	    ...
	    setSpeedUnit:function(speedUnit){
	    ...
	    copy: function ( source ) {
			...
			this.num = obj.num;
	        this.speedUnit = obj.speedUnit;
	        this.heightUnit = obj.heightUnit;
	        this.isCycle = obj.isCycle;
	...
	var AreaLibPath = function () {
	    ...
	    this.lineType = "Solid";
	    //椭圆
	    this.semiMinorAxis = 0; // 短边距离
	    this.semiMajorAxis = 0; // 长边距离
	    this.angle = 0; // 角度
	};
	AreaLibPath.prototype = Object.assign({
	    ...
	    setLabelCenter:function (labelCenter){
	        this.labelCenter = labelCenter;
	    },
	    getLabelCenter: function(){
	    ...
	    copy: function (source) {
	        ...
	        this.ascription = obj.ascription; //属方
	        this.color = obj.color;
	        this.drawType = obj.drawType;
	        this.opacity = obj.opacity;
	        this.labelCenter = obj.labelCenter;
	        this.lineWidth = obj.lineWidth;
	        this.lineType = obj.lineType;
	        //椭圆
	        this.semiMinorAxis = obj.semiMinorAxis; // 短边距离
	        this.semiMajorAxis = obj.semiMajorAxis; // 长边距离
	        this.angle = obj.angle; // 角度
	        return this;
	    },
	    clone: function () {
	    ...
	    toJson: function () {
	        ...
	        let json = JSON.parse(JSON.stringify(source));
	        return json;
	    }
	});
	...
	AreaLibPathList.prototype = Object.assign({
	  ...
	  readJson: function (json) {
	    ...
	    for (let i = 0; i < json.length; i++) {
	      ...
	      let _AreaLibPath = new AreaLibPath();
	      _AreaLibPath = Object.assign(_AreaLibPath, element);
	      // _AreaLibPath.setList(element.areaPointlist);
	      // _AreaLibPath.setName(element.name);
	      // _AreaLibPath.setNum(element.num);
	      // _AreaLibPath.setType(element.type);
	      // _AreaLibPath.setAscription(element.ascription);
	      // _AreaLibPath.setColor(element.color);
	      this.addChild(_AreaLibPath);
	    }
	  },
	});
	...
	RoutePathList.prototype = Object.assign({
	    ...
	    removeChildById(id){
	        let child = this.list.find(item=>item.id == id);
	        if (child) {
	            for (let i = 0; i < this.list.length; i++) {
	    ...
	    writeNavPath: function (node, navPath) {
	        ...
	        if (navPointCount > 0) {
	            ...
	            node_navPath.appendChild(points_ele);
	            for (let k = 0; k < navPointCount; k++) {
	                var navPoint = navPath.getNavPointByIndex(k);
	    ...
	    readJson: function (json) {
	        ...
	        for (let i = 0; i < json.length; i++) {
	            ...
	            _RoutePath = Object.assign(_RoutePath, element);
	            // _RoutePath.setNavPointList(element.NavPointlist);
	            // _RoutePath.setName(element.name);
	            // _RoutePath.setNum(element.num);
	            // _RoutePath.setId(element.id);
	            // _RoutePath.setWidth(element.width);
	            // _RoutePath.setColor(element.color);
	            this.addChild(_RoutePath);
	        }
	    }
	});
	 ...
	 * @Date: 2024-01-29 17:54:16
	 * @LastEditors: 谢小宇 xiexiaoyu@piesat.cn
	 * @LastEditTime: 2024-06-05 09:42:05
	 * @FilePath: \sim-web\src\environment\environment.js
	 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
	 */
	var environment$1 = function () {
	  this.id = -1;
	  this.cloud = { minHeight: 2000, maxHeight: 5000, concentration: 0.05 };
	  this.land = { cover: "general", formation: "level" };
	  this.rain = { rate: 0, altitudeLimit: 0 };
	  this.sea = { state: "0" };
	  this.maximum_range = "";
	...
	environment$1.prototype = Object.assign(
	  {
	    setFileIdList: function (fileIdList) {
	    ...
	    setMaximum_range: function (maximum_range) {
	      this.maximum_range = maximum_range;
	    },
	    getMaximum_range: function () {
	  ...
	  },
	  {
	    copy: function (source) {
	      ...
	      this.rain = obj.rain;
	      this.sea = obj.sea;
	      this.maximum_range = obj.maximum_range;

	      this.fileIdList = obj.fileIdList;

	      this.drawOptions = obj.drawOptions;
	    ...
	    toJson: function () {
	      let source = this.clone();
	      if(source.drawOptions.viewer){
	        source.drawOptions.viewer = null;
	      }      let json = JSON.parse(JSON.stringify(source));
	      return json;
	    },
	  }
	...
	environmentList.prototype = Object.assign({
	    ...
	    readJson(json){
	        ...
	        for (let i = 0; i < json.length; i++) {
	            const element = json[i];
	            let _environment = new environment$1();
	            _environment = Object.assign(_environment, element);
	            // _environment.setId(element.id);
	            // _environment.setCloud(element.cloud);
	            // _environment.setLand(element.land);
	            // _environment.setRain(element.rain);
	            // _environment.setSea(element.sea);
	            // _environment.setDrawOptions(element.drawOptions);
	            // _environment.setFileIdList(element.fileIdList);
	            this.addChild(_environment);
	        }
	    },
	    clearChild(){
	        this.list = [];
	...
	/*
	 * @Author: 谢小宇 xiexiaoyu@piesat.cn
	 * @Date: 2024-02-22 13:42:24
	 * @LastEditors: 谢小宇 xiexiaoyu@piesat.cn
	 * @LastEditTime: 2024-09-13 14:35:36
	 * @FilePath: \sim-web\src\network\network.js
	 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
	 */
	...
	network.prototype = Object.assign(
	  {
	    setId: function (id) {
	      this.id = id;
	    },
	  ...
	  },
	  {
	    copy: function (source) {
	      ...
	      this.type = obj.type;
	      this.side = obj.side;
	      this.channle = obj.channle;

	      return this;
	    },
	    clone: function () {
	      return new network().copy(this);
	    },
	    toJson: function () {
	      let source = this.clone();
	      let json = JSON.parse(JSON.stringify(source));
	...
	networkList.prototype = Object.assign({
	  getList: function () {
	    return this.list;
	  },
	  ...
	  setChildByIndex: function (index, child) {
	    this.list.splice(index, 1, child);
	  },
	  removeChildByIndex: function (index) {
	    this.list.splice(index, 1);
	  },
	  ...
	  readJson(json) {
	    ...
	    for (let i = 0; i < json.length; i++) {
	      ...
	      _network = Object.assign(_network, element);
	      // _network.setId(element.id);
	      // _network.setName(element.name);
	      // _network.setType(element.type);
	      // _network.setSide(element.side);
	      this.addChild(_network);
	    }
	  },
	  writeEnvironmentList: function (element, node) {},
	  writeXml: function (element) {
	...
	CGFEntity.prototype = Object.assign(
	  {
	    ...
	    setSixFree: function (sixFree) {
	      this.sixFree = sixFree;
	    },
	    getOilVolume: function () {
	      return this.oilVolume;
	    },
	    setOilVolume: function (oilVolume) {
	  ...
	  },
	  {
	    ...
	    initGeoData: function (geoData) {
	      ...
	      for (
	        let interest_i = 0;
	        interest_i < interestPoints.length;
	        interest_i++
	      ) {
	    ...
	    readXml: function (element) {
	      ...
	      if (nodeName == "指挥") {
	        ...
	        var strId = element.getAttribute("标识");
	        ...
	        for (let i = 0; i < childElements.length; ++i) {
	          ...
	          if (
	            childElement.nodeName == "指挥" ||
	            childElement.nodeName == "兵力"
	          ) {
	        ...
	        if (hangingBullet) {
	          ...
	          for (let i = 0; i < arms.length; i++) {
	            ...
	            for (let k = 0; k < ammunition_node.length; k++) {
	              var serialNumber_ammunition =
	                ammunition_node[k].getAttribute("序号");
	              var name_ammunition = ammunition_node[k].getAttribute("名称");
	              var quantity_ammunition = Number(
	                ammunition_node[k].getAttribute("数量")
	              );
	              ammunition.push({
	    ...
	    writeXml: function (xmlObject, navPathNodes) {
	      ...
	      if (this.getNodeType() == NodeUnit.NodeGroup) {
	        ...
	        if (window.navPath_value > 0) {
	          if (
	            (this.getModelId() == 2125770525 ||
	              this.getModelId() == 2118634858 ||
	              this.getModelId() == 2135514786) &&
	            this.side == 1
	          ) {
	          ...
	          if (hangingBullet.length > 0) {
	            ...
	            for (let i = 0; i < hangingBullet.length; i++) {
	              ...
	              for (let j = 0; j < ammunition.length; j++) {
	                ...
	                node_arms.append("\n\t");
	                node_ammunition.setAttribute(
	                  "序号",
	                  ammunition[j].serialNumber_ammunition
	                );
	                node_ammunition.setAttribute(
	                  "名称",
	                  ammunition[j].name_ammunition
	                );
	                node_ammunition.setAttribute(
	                  "数量",
	                  ammunition[j].quantity_ammunition
	                );
	              }
	            }
	          }
	    ...
	    writeTxt: function (platforms, platformTypes) {
	      ...
	      if (this.getNodeType() == NodeUnit.NodeGroup) {
	        //if(this.parent.name != "红方" && this.parent.name != "蓝方" )
	        {
	          let platform = new Platform();

	          platform.initPlatform(this.getName(), "FLIGHT_LEAD");
	          if (this.parent.name == "红方" || this.parent.name == "蓝方") {
	            platform.ProcessInput(
	              "command_chain " + sideToSim(this.side) + "_chain" + " SELF"
	            );
	          } else {
	            platform.ProcessInput(
	              "command_chain " +
	          ...
	          platform.ProcessInput(
	            "position " + this.position.y + "n " + this.position.x + "e "
	          );
	          ...
	          {
	            //设置通信
	            let sTx = `add comm blue_comm TEAM_DATALINK
                            network_name blue_net
                            internal_link data_mgr
                            internal_link task_mgr
                            internal_link perception
                        end_comm`;
	            platform.ProcessInput(sTx);
	          }
	          platforms.push(platform);
	          platformTypes.push("FLIGHT_LEAD");
	          for (let i = 0; i < this.children.length; i++) {
	            this.children[i].writeTxt(platforms, platformTypes);
	          }
	        }
	        ...
	        {
	          //设置武器
	          if (this.getHangingBullet().length > 0) {
	            let weapons = this.getHangingBullet();
	            for (let wea_i = 0; wea_i < weapons.length; wea_i++) {
	              let weapon = weapons[wea_i];
	              let sWeapon =
	                "add weapon " + weapon.type + " " + weapon.computer + "\n";
	              sWeapon = sWeapon + "\t quantity " + weapon.number + "\n";
	              sWeapon = sWeapon + "end_weapon";
	              platform.ProcessInput(sWeapon);
	            }
	          }
	        }
	        {
	          //设置雷达
	          if (this.getSensor().length > 0) {
	            let sensors = this.getSensor();
	            for (let sen_i = 0; sen_i < sensors.length; sen_i++) {
	              let sensor = sensors[sen_i];
	              let sSensor =
	                "add sensor " + sensor.name + " " + sensor.type + "\n";
	              sSensor = sSensor + "\t on " + "\n";
	              sSensor = sSensor + "end_sensor";
	              platform.ProcessInput(sSensor);
	            }
	          }
	        }
	        {
	    ...
	    copy: function (source) {
	      ...
	      this.parentCid = obj.parentCid;

	      this.modelUrl = obj.modelUrl;
	      this.imageUrl = obj.imageUrl;

	      this.sensorList = obj.sensorList; //传感器
	      this.weaponList = obj.weaponList; //武器
	      this.commandChain = obj.commandChain;
	      this.processorList = obj.processorList;
	      this.communicateList = obj.communicateList;
	      ...
	      this.oilVolume = obj.oilVolume;

	      this.sixFree = obj.sixFree;
	    ...
	    toJson: function () {
	      ...
	      source.children = [];
	      for (let i = 0; i < this.children.length; i++) {
	        source.children.push(this.children[i].toJson());
	      }
	      let json = JSON.parse(JSON.stringify(source));
	      return json;
	    },
	    readJSON: function () {
	      let source = Object.assign({}, this);
	      this.children = [];
	      for (let k = 0; k < source.children.length; k++) {
	        // this.loadJSON(vecNodes[i].children)
	        let pCGFEntity = new CGFEntity(null, null);
	        pCGFEntity = Object.assign(pCGFEntity, source.children[k]);
	        if (pCGFEntity.navPath) {
	          let _NavPath = new NavPath();
	          _NavPath = Object.assign(_NavPath, pCGFEntity.navPath);
	          // _NavPath.setNavPointList(pCGFEntity.navPath.NavPointlist);
	          // _NavPath.SetLoopIndex(pCGFEntity.navPath.loopIndex);
	          pCGFEntity.setNavPath(_NavPath);
	        }
	        pCGFEntity.readJSON();
	        this.addChild(pCGFEntity);
	      }
	    },
	  }
	);
	...
	CGFDataModel.prototype.loadXmlString = function (xmlString) {
	    ...
	    _this.file.getElementsByTagName('环境库');

	    this.environmentNodes = new environmentList();
	    ...
	    _this.file.getElementsByTagName('网络库');

	    this.networkNodes = new networkList();
	};
	...
	CGFDataModel.prototype.writeXml = function () {
	    ...
	    let routePathNode = this.file.getElementsByTagName("路线库");
	    if (routePathNode) {
	        if (routePathNode.length < 1) ; else {
	            let routePathList = routePathNode[0];
	            for (let routenode_i = routePathList.children.length - 1; routenode_i >= 0; routenode_i--) {
	                let node = routePathList.children[routenode_i];
	                node.remove();
	            }
	            this.routePathNodes.writeXml(routePathList);
	        }
	    }

	    let areaPathNode = this.file.getElementsByTagName("区域库");
	    if (areaPathNode) {
	    ...
	    let networkNode = this.file.getElementsByTagName("网络库");
	    if (networkNode) {
	        if (networkNode.length < 1) ; else {
	            let networkList = networkNode[0];
	            for (let network_i = networkList.children.length - 1; network_i >= 0; network_i--) {
	                let node = networkList.children[network_i];
	                node.remove();
	            }
	            this.networkNodes.writeXml(networkList);
	        }
	    }
	};

	CGFDataModel.prototype.loadJSON = function(vecNodes){
	    this.vecNodes = [];
	    for(let i=0;i<vecNodes.length;i++){
	        let pCGFEntity = new CGFEntity(null,null);
	        pCGFEntity = Object.assign(pCGFEntity,vecNodes[i]);
	        pCGFEntity.readJSON();
	        this.vecNodes.push(pCGFEntity);
	    }
	};
	...
	CGFDataModel.prototype.saveJson = function(){
	    let json = {
	        vecNodes: [],
	        environmentNodes: [],
	        networkNodes: [],
	    ...
	    for (let i = 0; i < this.vecNodes.length; i++) {
	        var pCGFEntity = this.vecNodes[i];
	        json.vecNodes.push(pCGFEntity.toJson());
	    }
	    let environmentList = this.environmentNodes.getList();
	    for (let j = 0; j < environmentList.length; j++) {
	        var environment = environmentList[j];
	        json.environmentNodes.push(environment.toJson());
	    }
	    let networkList = this.networkNodes.getList();
	    for (let j = 0; j < networkList.length; j++) {
	        var network = networkList[j];
	        json.networkNodes.push(network.toJson());
	    }
	    let areaPathList = this.areaPathNodes.getList();
	    for (let j = 0; j < areaPathList.length; j++) {
	        var areaPath = areaPathList[j];
	        json.areaPathNodes.push(areaPath.toJson());
	    }
	    let routePathList = this.routePathNodes.getList();
	    for (let j = 0; j < routePathList.length; j++) {
	        var routePath = routePathList[j];
	        json.routePathNodes.push(routePath.toJson());
	    }
	    console.log(json);
	...
	Object.assign({
	    ...
	    intMove:function(modeType){
	        // mover WSF_GROUND_MOVER 
	        // end_mover
	        let smove = "mover " + modeType;
	        let emove = "end_mover";
	        this.movers.push(smove);
	        this.movers.push(emove);       
	    },
	    setMove:function(s){
	        if(this.movers.length == 0){
	            this.intMove("WSF_AIR_MOVER");
	        }
	        this.movers.splice(this.movers.length-1,0,s);
	        console.log(this.movers);
	    },
	    initPlatformType:function(type){
	        let sPlatform = "platform_type " + type + " WSF_PLATFORM";
	        this.aPlatformType.push(sPlatform);
	        this.aPlatformType.push("end_platform_type");
	    },
	    ProcessInput:function(s){
	        this.aPlatformType.splice(this.aPlatformType.length-1,0,s);
	        console.log(this.aPlatformType);
	    },
	    getString:function(){
	        let smover = this.movers.join("\n");
	        this.ProcessInput(smover);
	        return this.aPlatformType.join("\n")
	    }
	...
	Simulation.prototype = Object.assign({
	    setPlatformType:function(type){
	   
	    },
	    setOutput:function(){
	       let entStart =  "event_output";
	       let file =" file output/jacksonabad.evt";
	       let entEnd = "end_event_output";
	       this.ProcessInput(entStart);
	       this.ProcessInput(file);
	       this.ProcessInput(entEnd);
	    },
	    addInclude:function(){

	    },

	    initSimulation:function(time){
	        let file = "file_path .";
	        let last = "end_time "+time+" hr";

	        this.aSimulation.push(file);
	        this.aSimulation.push(last);
	    },
	    ProcessInput:function(s){
	        this.aSimulation.splice( this.aSimulation.length-1,0,s);
	    },

	    save:function(){
	        {//添加platform type
	            let platformTypes = this.types;

	            for(let i=0;i<platformTypes.length;i++){
	                let PlatformType= platformTypes[i];
	                let sPlatformType = "include_once platforms/" + PlatformType +".txt";
	                this.ProcessInput(sPlatformType);
	            }

	            // for(let i=0;i< platformTypes.length;i++){
	            //     let pType = platformTypes[i];
	            //     let valList = pType.form;
	            //     let pPlatformType = new PlatformType();
	            //     pPlatformType.initPlatformType(pType.typeId)
	            //     for(let j=0;j<valList.length;j++){
	            //         let temp = valList[j];
	            //         if(temp.tabsName == "动态"){
	            //             if(pPlatformType.movers.length>0){
	        
	            //             }else{
	            //                 pPlatformType.intMove("WSF_GROUND_MOVER")
	            //             }
	            //             let sVal = temp.fields + " " + temp.value + " " + temp.unit
	            //             pPlatformType.setMove(sVal)
	            //         }
	            //     }
	            //     let sPlatformType = pPlatformType.getString();
	            //     this.ProcessInput(sPlatformType)
	            // }
	        }
	        ...
	        {//添加platform
	            ...
	            for(let pi = 0; pi<platforms.length;pi++){
	                let pPlatform = platforms[pi];
	                let sPlatform = pPlatform.aPlatform.join("\n");
	                this.ProcessInput(sPlatform);
	            }
	        }

	        console.log(this.aSimulation);

	        let sSimulation = this.aSimulation.join("\n");
	        console.log(sSimulation);
	        this.downtxt(sSimulation);
	    },
	    downtxt:function(sSimulation){
	        // 创建一个Blob对象，将数组转换为文本  
	        var blob = new Blob([sSimulation], {type: "text/plain;charset=utf-8"});  
	 ...
	 * @Date: 2024-01-29 17:54:16
	 * @LastEditors: 谢小宇 xiexiaoyu@piesat.cn
	 * @LastEditTime: 2024-02-22 13:21:06
	 * @FilePath: \sim-web\src\SimDataModel\UseFunction.js
	 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
	 */
	...
	function SideTOCommandChain(side) {
	    let commandChain = "other_chain";
	    if (side == 1) {
	        commandChain = "red_chain";
	    } else if (side == 2) {
	        commandChain = "blue_chain";
	    } else if (side == 3) {
	        commandChain = "green_chain";
	    } else if (side == 4) {
	        commandChain = "orange_chain";
	    } else if (side == 5) {
	        commandChain = "yellow_chain";
	    } else {
	        commandChain = "purple_chain";
	    }
	    return commandChain;
	}
	...
	exports.SideTOCommandChain = SideTOCommandChain;
