          ...
          <template #default="scope">
            <span>
              <el-input
                ...
                @change="changeName(scope.row, scope.$index)"
              />
            </span>
          </template>
        </el-table-column>
...
let vueData = reactive({
  tableHeader: [{ prop: 'type', label: '类型' }],
  tableData: [
...
const changeName = (row, rowIndex) => {
  console.log('changeName', row)
}
 ...
 */
const addRadar = () => {
  console.log('addRadar', vueData.optionItem)
  if (vueData.optionItem.type) {
    vueData.tableData.push({ ...vueData.optionItem })
    let pCGF = store.getters.get_CGF_data
    pCGF.sensor = vueData.tableData
  }
}
</script>
