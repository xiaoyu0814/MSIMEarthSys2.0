...
const CreateFlag = function (viewer, resultList, options, callBack) {
  ...
  handler.setInputAction(function (evt) {
    ...
    if (!window.MSIMEarth.defined(flagEntity)) {
      flagEntity = viewer.entities.add({
        id: id,
        name: flagType[type],
        polygon: {
          hierarchy: new window.MSIMEarth.CallbackProperty(function () {
            let arrow = []
            let res = fineArrow()
            for (let i = 0; i < res.length; i++) {
              let cart3 = new window.MSIMEarth.Cartesian3(
                res[i].x,
                res[i].y,
                res[i].z
              )
            ...
            return new window.MSIMEarth.PolygonHierarchy(arrow)
          }, false),
  ...
  function fineArrow() {
    ...
    if (points < 2) {
      ...
      return window.MSIMEarth.Cartesian3.fromDegreesArray(
        [].concat.apply([], arcArr)
      )
    }
  }
