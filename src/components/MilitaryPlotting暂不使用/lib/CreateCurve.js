  ...
  handler.setInputAction(function (movement) {
    ...
    if (anchorpoints.length > 0) {
      if (!window.MSIMEarth.defined(polyline)) {
        ...
        linePoints = window.MSIMEarth.Cartesian3.fromDegreesArray(geoPoints)
        polyline = viewer.entities.add({
          name: 'Curve',
          id: id,
          polyline: {
            positions: new window.MSIMEarth.CallbackProperty(function () {
              return linePoints
            }, false),
            width: width,
            material: color
        ...
        linePoints = window.MSIMEarth.Cartesian3.fromDegreesArray(GeoPoints1)
      }
    }
