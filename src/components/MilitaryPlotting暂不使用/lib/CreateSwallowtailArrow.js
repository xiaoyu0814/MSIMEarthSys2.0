    ...
    if (anchorpoints.length > 1) {
      if (!window.MSIMEarth.defined(polygon)) {
        ...
        SwallowtailArrowPoints =
          window.MSIMEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints)
        polygon.polygon.hierarchy = new window.MSIMEarth.CallbackProperty(
          function () {
            return new window.MSIMEarth.PolygonHierarchy(SwallowtailArrowPoints)
          },
          false
        )
        ...
        SwallowtailArrowPoints =
          window.MSIMEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints1)
      }
    } else {
      return
    }
...
function plotingSwallowtailArrow(viewer, anchorpoints, color) {
  ...
  let polygon = viewer.entities.add({
    ...
    polygon: {
      hierarchy: window.MSIMEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints),
      material: color
    }
  })
  return polygon
