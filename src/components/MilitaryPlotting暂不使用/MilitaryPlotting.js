...
import CreateAttackArrowD from './lib/CreateAttackArrowD'
...
class MilitaryPlotting {
   ...
   * @param {*} callBack 回调函数，可直接引用创建的对象
   */
  CreateAttackArrowD(options, callBack) {
    CreateAttackArrowD(
      this.viewer,
      this.resultObject.attackArrow,
      options,
  ...
  CreateLineArrow(options, callback) {
    CreateLineArrow(this.viewer, this.resultObject.lineArrow, options, callback)
  }
  /**
   * 创建燕尾箭头
