class BeautyToast {
  ...
  success({
    ...
    this.checkAndAddToastWrapper.then((toastWrapper) => {
      ...
      const toastIcon =
        icon == null
          ? `<svg width="${iconWidth ?? '32'}" height="${
              iconHeight ?? '32'
            }" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M27.2309 9.65179L11.731 25.1518L4.62679 18.0476L6.44803 16.2264L11.731 21.4964L25.4097 7.83054L27.2309 9.65179Z" fill="${
              iconColor ?? '#68D84C'
            }"/></svg>`
          : `<i class="${icon}" style="font-size: ${
              iconWidth != null ? iconSize + 'px;' : '52px;'
            }"></i>`
  ...
  processToast = ({
    ...
    new Promise((resolve, reject) => {
      ...
      .then(async (toastId) => {
        const specificToast = document.querySelector(
          `.beautyToast[toast-id="${toastId}"]`
        )
        setTimeout(() => {
      ...
      .then(async (toastId) => {
        ...
        const timeoutInterval = setInterval(async () => {
          ...
          specificToast.querySelector('.beautyToast-progress').style.width =
            percentage + '%'
          if (remainingTime == 0) {
            clearInterval(timeoutInterval)
            await closeToast(toastId, onClose)
            const toastWrapper = document.querySelector('.beautyToast-wrapper')
            if (toastWrapper.hasChildNodes()) {
              let firstId = toastWrapper.childNodes[0].getAttribute('toast-id')
              if (firstId != toastId) {
                closeToast(firstId, onClose) // 删除第一个元素
              }
            }
          }
        }, 200)
  ...
  toastGenerator({
    ...
    const toast = `<div class="beautyToast ${
      darkTheme ? 'beautyToast-dark' : ''
    } beautyToast-${toastType}  ${
      !closeButton ? 'no-close' : ''
    }" toast-id="${toastId}" ${id != null ? 'id="' + id + '"' : ''} style="${
      backgroundColor != null
        ? 'background-color: ' + backgroundColor + '!important;'
        : ''
    }"><div class="beautyToast-topbar" ${
      topbarColor != null
        ? 'style="background-color:' + topbarColor + '!important;"'
        : ''
    }></div><div class="beautyToast-body"><div class="beautyToast-title-group"><div class="beautyToast-icon">${toastIcon}</div><h5 class="beautyToast-title" style="${
      titleColor != null ? 'color: ' + titleColor + '!important;' : ''
    }">${title}</h5></div><p class="beautyToast-text ${
      animation != null ? 'beautyToast-text-animation' : ''
    }" style="${
      messageColor != null ? 'color: ' + messageColor + '!important;' : ''
    }">${message}</p></div>${
      closeButton
        ? `<button class="beautyToast-close" onclick="closeToast(${toastId}, ${onClose})">&times;</button>`
        : ''
    }<div class="beautyToast-progress" style="${
      progressBarColor != null
...
globalThis.closeToast = closeToast
