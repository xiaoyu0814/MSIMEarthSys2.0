...
const pickSameName = (option, name, type) => {
  if (
    state.redAndBlueRadarOption.series &&
    state.redAndBlueRadarOption.series.length > 0 &&
    state.redAndBlueRadarOption.series[0].data &&
    state.redAndBlueRadarOption.series[0].data.length > 0 &&
    state.redAndBlueRadarOption.series[0].data[0].children
  ) {
  ...
  return new Promise((resolve, reject) => {
    if (store.state.sceneModule.isReplayType) {
      ...
      getResultReplay().then((res) => {
        state.redAndBlueRadarOption = makeOption([res.blue.sideTypeJson])
