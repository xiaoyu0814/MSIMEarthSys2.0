class SlopeAspectTool {
  constructor(viewer) {
    ...
    this.enable = false
    this.resultArray = []
    this.terrainLayer = viewer.terrainLayer
    this.pointArray = []
  }
  getEnable() {
    return this.enable
  ...
  drawPlygon() {
      ...
      if (MSIMEarth.defined(earthPosition)) {
        if (activeShapePoints.length === 0) {
          ...
          var dynamicPositions = new MSIMEarth.CallbackProperty(function () {
            return new MSIMEarth.PolygonHierarchy(activeShapePoints)
          }, false)
    ...
    //
    this.handler.setInputAction(function (event) {
      if (MSIMEarth.defined(floatingPoint)) {
        var newPosition
  ...
  drawSlopePolygon(
    ...
    for (let j = 0; j < 10; j++) {
      for (var i = 0; i < 10; i++) {
        ...
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(hireacy[0].x, hireacy[0].y)
        )
        slopelineposition.push(
        ...
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(hireacy[1].x, hireacy[1].y)
        )
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(
        ...
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(hireacy[2].x, hireacy[2].y)
        )
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(
        ...
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(hireacy[3].x, hireacy[3].y)
        )
        slopelineposition.push(
          MSIMEarth.Cartographic.fromDegrees(
    ...
    var promise = MSIMEarth.sampleTerrainMostDetailed(
      terrainLayer,
      slopelineposition
    )
    var that = this
    MSIMEarth.when(promise, function (updatedPositions) {
      //拿到所有的高度数据
      ...
      for (let k = 0; k < updatedPositions.length / 8; k++) {
        ...
        if (slope == Math.abs(slope1)) {
          if (slope1 > 0) {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m].latitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 4].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 4].latitude)
            )
          } else {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 4].longitude),
          ...
          if (slope2 > 0) {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 1].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 1].latitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 5].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 5].latitude)
            )
          } else {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 5].longitude),
          ...
          if (slope3 > 0) {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 2].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 2].latitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 6].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 6].latitude)
            )
          } else {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 6].longitude),
          ...
          if (slope4 > 0) {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 3].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 3].latitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 7].longitude),
              MSIMEarth.Math.toDegrees(updatedPositions[m + 7].latitude)
            )
          } else {
            lineposition.push(
              MSIMEarth.Math.toDegrees(updatedPositions[m + 7].longitude),
        ...
        // console.log(MSIMEarth.Color)
        var result = that.viewer.entities.add({
          type: 'drawSloperectange',
          // rectangle: {
          //     coordinates: MSIMEarth.Rectangle.fromCartographicArray(cartographics)
          //     ,
          //     material: MSIMEarth.Color.fromCssColorString(slopecolor)

          // },
          polyline: {
            clampToGround: true,
            positions: MSIMEarth.Cartesian3.fromDegreesArray(lineposition),
            material: new MSIMEarth.PolylineArrowMaterialProperty(
              MSIMEarth.Color.RED
            ), // MSIMEarth.Color.BLUE.withAlpha(0.5) ,
            width: 10,
            distanceDisplayCondition: new MSIMEarth.DistanceDisplayCondition(
              0,
              updatedPositions[m + 4].height + 30000
            )
          }
        })
        // debugger
...
export default SlopeAspectTool
