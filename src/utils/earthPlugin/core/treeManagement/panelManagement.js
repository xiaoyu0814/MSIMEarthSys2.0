...
import { createTurbulence, removeTurbulence } from './methods/turbulence.js'

let cloud_interval = null
...
class panelManagement extends basicManagement {
      ...
      if (type == 'add') {
        ...
        switch (val.code) {
          ...
          case 'infraredDetectionZone': //红外探测区域
            ...
            break
          case 'wind': //风场
            // 后续会基于模型获取风场数据，数据信息会存到缓存中（最好后台基于当前实验存储）
          ...
          case 'windtj': //风场可调节
            ...
            const windDataPaths = [
              ...
              {
                name: '2025-08-23 17:12:33',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171233.json'
              },
              {
                name: '2025-08-23 17:13:34',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171334.json'
              },
              {
                name: '2025-08-23 17:14:35',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171435.json'
              },
              {
                name: '2025-08-23 17:15:36',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171536.json'
              },
              {
                name: '2025-08-23 17:16:37',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171637.json'
              },
              {
                name: '2025-08-23 17:17:38',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171738.json'
              },
              {
                name: '2025-08-23 17:18:40',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171840.json'
              },
              {
                name: '2025-08-23 17:19:41',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_171941.json'
              },
              {
                name: '2025-08-23 17:20:42',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_172042.json'
              },
              {
                name: '2025-08-23 17:21:43',
                path: '/static/data/json/wind_json_output/FYu_L2A_AFT_PPI_EX4_20250823_172143.json'
              }
            ]
                    ...
                    if (config.removeIsolatedPoints) {
                      ...
                      for (let dy = -1; dy <= 1; dy++) {
                        for (let dx = -1; dx <= 1; dx++) {
                          ...
                          const nSpeed = calculateWindSpeed(
                            data.u[nIdx],
                            data.v[nIdx]
                          )
                  ...
                  for (let x = 0; x < nx; x++) {
                    ...
                    for (let dy = -radius; dy <= radius; dy++) {
                      for (let dx = -radius; dx <= radius; dx++) {
                        ...
                        const nyVal = Math.max(0, Math.min(ny - 1, y + dy))
                        const nIdx = z * ny * nx + nyVal * nx + nxVal
                        const speed = calculateWindSpeed(
                          data.u[nIdx],
                          data.v[nIdx]
                        )
                        if (speed > config.minSpeedThreshold) {
            ...
            const reconstructByDensity = (data, nx, ny, nz, config) => {
              ...
              // 2. 查找局部最大值
              const localMaxima = config.preserveLocalMax
                ? findLocalMaxima(densityMap, nx, ny, nz)
                : null
              ...
              console.log('📊 密度重构完成', {
                ...
                移除点数: removedPoints,
                保留率:
                  ((keptPoints / (keptPoints + removedPoints)) * 100).toFixed(
                    1
                  ) + '%',
            ...
            const generateSeedPoints = (header, data, config) => {
              ...
              const seeds = []
              const startLevel = filterConfig.enableLevelFilter
                ? filterConfig.minLevel
                : 0
              const endLevel = filterConfig.enableLevelFilter
                ? filterConfig.maxLevel
                : nz - 1
              ...
              const spacing = Math.max(
                2,
                Math.floor(Math.sqrt((nx * ny) / config.streamlineCount))
              )
            ...
            const applyDensitySampling = (header, data, config) => {
              ...
              console.log(
                `📊 密度采样: ${nx}×${ny} → ${newNx}×${newNy} (间隔: ${density})`
              )
            ...
            const applySpeedThresholdMode = (header, data, config) => {
              ...
              console.log(
                `   ✓ 保留: ${keptPoints} 点 | ✗ 移除: ${removedPoints} 点`
              )
              return {
                u,
                v,
            ...
            const applyStreamlineFilter = (filteredData) => {
              ...
              if (filterConfig.streamlineMode === 'density') {
                ...
                filteredData.header.lo2 =
                  header.lo1 + (result.newNx - 1) * lonStep * density
                filteredData.header.la2 =
                  header.la1 + (result.newNy - 1) * latStep * density
              }
            ...
            const applyFilter = () => {
              ...
              if (filterConfig.enableLevelFilter) {
                ...
                currentFilteredData.header.levels = levels.slice(
                  startLevel,
                  endLevel + 1
                )
                if (header.levelHeights) {
                  currentFilteredData.header.levelHeights =
                    header.levelHeights.slice(startLevel, endLevel + 1)
                }
              }
              ...
              // 确定有效的x和y范围（基于区域筛选）
              let effectiveXStart = 0,
                effectiveXEnd = origNx - 1
              let effectiveYStart = 0,
                effectiveYEnd = origNy - 1
              let newNx = origNx,
                newNy = origNy
              let newLo1 = header.lo1,
                newLo2 = header.lo2
              let newLa1 = header.la1,
                newLa2 = header.la2
              ...
              if (filterConfig.enableAreaFilter) {
                ...
                console.log('📍 区域筛选参数', {
                  用户输入: {
                    lon: `${filterConfig.areaMinLon}-${filterConfig.areaMaxLon}`,
                    lat: `${filterConfig.areaMinLat}-${filterConfig.areaMaxLat}`
                  },
                  实际使用: {
                  ...
                  原始范围: {
                    lon: `${header.lo1}-${header.lo2}`,
                    lat: `${header.la1}-${header.la2}`
                  }
                })
                ...
                if (origNx > 1 && origNy > 1) {
                  ...
                  effectiveXStart = Math.max(
                    0,
                    Math.floor((minLon - header.lo1) / lonStep)
                  )
                  effectiveXEnd = Math.min(
                    origNx - 1,
                    Math.ceil((maxLon - header.lo1) / lonStep)
                  )
                  effectiveYStart = Math.max(
                    0,
                    Math.floor((minLat - header.la1) / latStep)
                  )
                  effectiveYEnd = Math.min(
                    origNy - 1,
                    Math.ceil((maxLat - header.la1) / latStep)
                  )
                  ...
                  if (effectiveXStart > effectiveXEnd) {
                    console.error(
                      `❌ X轴索引无效: ${effectiveXStart} > ${effectiveXEnd}，使用全范围`
                    )
                    effectiveXStart = 0
                    effectiveXEnd = origNx - 1
                  }
                  if (effectiveYStart > effectiveYEnd) {
                    console.error(
                      `❌ Y轴索引无效: ${effectiveYStart} > ${effectiveYEnd}，使用全范围`
                    )
                    effectiveYStart = 0
                    effectiveYEnd = origNy - 1
                  }
                  ...
                  console.warn(
                    '⚠️ 原始网格维度太小 (nx≤1 或 ny≤1)，无法进行区域筛选'
                  )
                }
              }
                    ...
                    if (filterConfig.enableDirectionFilter) {
                      ...
                      if (speed > 0.1) {
                        ...
                        if (
                          !isDirectionInRange(
                            direction,
                            filterConfig.targetWindDirection,
                            filterConfig.windDirectionTolerance
                          )
                        ) {
              ...
              console.log('✅ 筛选完成', {
                ...
                区域范围: `${newLo1.toFixed(4)}-${newLo2.toFixed(
                  4
                )}E, ${newLa1.toFixed(4)}-${newLa2.toFixed(4)}N`,
                header信息: {
              ...
              if (u.length !== expectedSize) {
                console.error(
                  `❌ 数据大小不一致！期望 ${expectedSize}，实际 ${u.length}`
                )
              } else {
            ...
            const updateWindLegend = () => {
              ...
              // 添加筛选信息
              if (
                filterConfig.enableLevelFilter ||
                filterConfig.enableDirectionFilter ||
                filterConfig.enableAreaFilter ||
                filterConfig.enableStreamlineFilter ||
                filterConfig.enableSmoothing ||
                filterConfig.enableDensityFilter
              ) {
                legendHTML += `
                  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.2); font-size: 10px; opacity: 0.8;">
                    ...
                    ${
                      filterConfig.enableLevelFilter
                        ? `<div>层级: ${
                            originalWindData?.header?.levels?.[
                              filterConfig.minLevel
                            ] || 0
                          }-${
                            originalWindData?.header?.levels?.[
                    ...
                    ${
                      filterConfig.enableDirectionFilter
                        ? `<div>风向: ${filterConfig.targetWindDirection}°±${filterConfig.windDirectionTolerance}°</div>`
                        : ''
                    }
                    ${
                      filterConfig.enableAreaFilter
                        ? `<div>区域: ${filterConfig.areaMinLon.toFixed(
                            3
                          )}°-${filterConfig.areaMaxLon.toFixed(
                            3
                          )}°E, ${filterConfig.areaMinLat.toFixed(
                            3
                          )}°-${filterConfig.areaMaxLat.toFixed(3)}°N</div>`
                        : ''
                    }
                    ${
                      filterConfig.enableStreamlineFilter
                        ? `<div>流线: ${
                            filterConfig.streamlineMode === 'density'
                              ? '密度采样×' + filterConfig.streamlineDensity
                              : filterConfig.streamlineMode === 'path'
                              ? filterConfig.streamlineCount + '条路径'
                              : '风速≥' +
                                filterConfig.streamlineMinSpeed +
                                'm/s'
                          }</div>`
                        : ''
                    }
                    ${
                      filterConfig.enableSmoothing
                        ? `<div>平滑: 强度${filterConfig.smoothingStrength}${
                            filterConfig.removeIsolatedPoints ? '+去孤立点' : ''
                          }</div>`
                        : ''
                    }
                    ${
                      filterConfig.enableDensityFilter
                        ? `<div>密度: 阈值${filterConfig.densityThreshold} 半径${filterConfig.densityRadius}</div>`
                        : ''
                    }
                  </div>
                `
              ...
              if (wind3d.speedExaggerationEnabled) {
                legendHTML += `
                  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.2); font-size: 10px; opacity: 0.8;">
                    <div>速度系数: ${wind3d.speedFactor.toFixed(2)} | 厚度: ${(
                  wind3d.windThickness / 1000
                ).toFixed(0)}km</div>
                    <div>风速夸张: 开 | 模式: ${
                      wind3d.speedExaggerationMode
                    } | 倍率: ${wind3d.speedExaggerationMultiplier.toFixed(
                  1
                )}</div>
                    <div>范围: ${wind3d.speedExaggerationMin.toFixed(
                      1
                    )}-${wind3d.speedExaggerationMax.toFixed(1)}m/s</div>
                  </div>
                `
              } else {
                legendHTML += `
            ...
            try {
              ...
              windFolder
                .add(wind3d, 'windThickness', 1000, 50000)
                .name('风场厚度')
                .onChange(updateWindLegend)
              windFolder
                .add(wind3d, 'speedFactor', 0.1, 5)
                .name('速度系数')
                .onChange(updateWindLegend)
              windFolder.add(wind3d, 'cullSpeedMin', 1, 50).name('最小剔除速度')
              windFolder
                .add(wind3d, 'cullSpeedMax', 10, 200)
                .name('最大剔除速度')
              windFolder
                .add(wind3d, 'windSpeedMin', 0, 50)
                .name('最小风速显示')
                .onChange(updateWindLegend)
              windFolder
                .add(wind3d, 'windSpeedMax', 5, 100)
                .name('最大风速显示')
                .onChange(updateWindLegend)
              ...
              smoothingFolder
                .add(filterConfig, 'enableSmoothing')
                .name('启用平滑')
                .onChange(applyFilterAndUpdate)
              smoothingFolder
                .add(filterConfig, 'smoothingStrength', 1, 5, 1)
                .name('平滑强度')
                .onChange(applyFilterAndUpdate)
              smoothingFolder
                .add(filterConfig, 'minSpeedThreshold', 0, 5, 0.1)
                .name('最小风速阈值')
                .onChange(applyFilterAndUpdate)
              smoothingFolder
                .add(filterConfig, 'removeIsolatedPoints')
                .name('移除孤立点')
                .onChange(applyFilterAndUpdate)
              ...
              densityFolder
                .add(filterConfig, 'enableDensityFilter')
                .name('启用密度筛选')
                .onChange(applyFilterAndUpdate)
              densityFolder
                .add(filterConfig, 'densityRadius', 1, 8, 1)
                .name('密度计算半径')
                .onChange(applyFilterAndUpdate)
              densityFolder
                .add(filterConfig, 'densityThreshold', 0.1, 0.8, 0.05)
                .name('密度保留阈值')
                .onChange(applyFilterAndUpdate)
              densityFolder
                .add(filterConfig, 'preserveLocalMax')
                .name('保留局部最大值')
                .onChange(applyFilterAndUpdate)
              ...
              const dataSelector = windFolder
                .add(windDataConfig, 'currentDataIndex', dataNames)
                .name('数据源')
              dataSelector.onChange(async (selectedName) => {
                const selectedIndex = windDataPaths.findIndex(
                  (d) => d.name === selectedName
                )
                if (
                  selectedIndex >= 0 &&
                  selectedIndex < windDataPaths.length
                ) {
              ...
              const updateLevelCheckboxes = () => {
                ...
                if (wind3d.levelHeights && wind3d.levelHeights.length > 0) {
                  ...
                  minHeightController = levelFilterFolder
                    .add(heightConfig, 'displayHeightMin', minH, maxH, 10)
                    .name('最小高度(m)')
                    .onChange(() => {
                      if (
                        heightConfig.displayHeightMin >
                        heightConfig.displayHeightMax
                      ) {
                        heightConfig.displayHeightMax =
                          heightConfig.displayHeightMin
                      }
                  ...
                  maxHeightController = levelFilterFolder
                    .add(heightConfig, 'displayHeightMax', minH, maxH, 10)
                    .name('最大高度(m)')
                    .onChange(() => {
                      if (
                        heightConfig.displayHeightMax <
                ...
                if (wind3d.levelHeights && wind3d.levelHeights.length > 0) {
                  ...
                  wind3d.levelHeights.forEach((height) => {
                    const levelObj = {
                      [`${height}m`]: wind3d.enabledLevels.includes(height)
                    }
                    const controller = levelFilterFolder
                      .add(levelObj, `${height}m`)
                      .name(`${height}m`)
                      .onChange((enabled) => {
                        if (enabled) {
              ...
              // 启用/禁用层级过滤
              levelFilterFolder
                .add(wind3d, 'levelFilterEnabled')
                .name('启用高度过滤')
                .onChange(updateLevelCheckboxes)

              // 等待数据加载后初始化高度范围
              const initHeightConfig = () => {
                if (wind3d.levelHeights && wind3d.levelHeights.length > 0) {
                  heightConfig.displayHeightMin = Math.min(
                    ...wind3d.levelHeights
                  )
                  heightConfig.displayHeightMax = Math.max(
                    ...wind3d.levelHeights
                  )
                  wind3d.displayHeightMin = heightConfig.displayHeightMin
                  wind3d.displayHeightMax = heightConfig.displayHeightMax
              ...
              directionFilterFolder
                .add(filterConfig, 'enableDirectionFilter')
                .name('启用风向过滤')
                .onChange(applyFilterAndUpdate)
              directionFilterFolder
                .add(filterConfig, 'targetWindDirection', 0, 360, 1)
                .name('目标风向(°)')
                .onChange(applyFilterAndUpdate)
              directionFilterFolder
                .add(filterConfig, 'windDirectionTolerance', 1, 180, 1)
                .name('容差(°)')
                .onChange(applyFilterAndUpdate)
              ...
              areaFilterFolder
                .add(filterConfig, 'enableAreaFilter')
                .name('启用区域过滤')
                .onChange(applyFilterAndUpdate)
              areaFilterFolder
                .add(
              ...
              streamlineFolder
                .add(filterConfig, 'enableStreamlineFilter')
                .name('启用流线筛选')
                .onChange(applyFilterAndUpdate)

              const streamlineModeOptions = {
                密度采样: 'density',
                流线路径: 'path',
                风速阈值: 'speed'
              }
              streamlineFolder
                .add(filterConfig, 'streamlineMode', streamlineModeOptions)
                .name('筛选模式')
                .onChange(applyFilterAndUpdate)

              streamlineFolder
                .add(filterConfig, 'streamlineDensity', 1, 10, 1)
                .name('采样间隔')
                .onChange(applyFilterAndUpdate)
              streamlineFolder
                .add(filterConfig, 'streamlineCount', 3, 20, 1)
                .name('流线数量')
                .onChange(applyFilterAndUpdate)
              streamlineFolder
                .add(filterConfig, 'streamlineMinSpeed', 0, 30, 0.5)
                .name('最小风速(m/s)')
                .onChange(applyFilterAndUpdate)
              streamlineFolder
                .add(filterConfig, 'streamlineMaxLength', 20, 200, 10)
                .name('最大长度(点)')
                .onChange(applyFilterAndUpdate)
              streamlineFolder
                .add(filterConfig, 'streamlineStepSize', 0.1, 2, 0.1)
                .name('追踪步长')
                .onChange(applyFilterAndUpdate)
          ...
          case 'humiditytj': //湿度可调节
            let humidityOptions = {}
            humidityOptions.name =
              weatherDataConfig.humidity.texturePaths[1].name
            humidityOptions.path =
              weatherDataConfig.humidity.texturePaths[1].path
            ...
            const humidityTexturePaths = [
              {
                name: '00:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0000_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '01:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0100_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '02:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0200_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '03:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0300_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '04:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0400_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '05:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0500_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '06:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0600_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '07:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0700_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '08:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0800_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '09:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_0900_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '10:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1000_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '11:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1100_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '12:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1200_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '13:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1300_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '14:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1400_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '15:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1500_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '16:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1600_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '17:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1700_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '18:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1800_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '19:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_1900_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '20:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_2000_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '21:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_2100_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '22:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_2200_z_interp_crop_100m_lat_vertical_16x16_green.png'
              },
              {
                name: '23:00',
                path: '/static/image/texture/WETpicture_GREEN_100m/RH_2024-02-05_2300_z_interp_crop_100m_lat_vertical_16x16_green.png'
              }
            ]
            ...
            const humidityPrimitive =
              DC.createHumidityTextureAliasOD(humidityConfig)
            window.humidityInstance = {
              DC,
              primitive: humidityPrimitive,
              config: humidityConfig
            }

            const humidityCenterLon =
              (humidityConfig.xmin + humidityConfig.xmax) / 2
            const humidityCenterLat =
              (humidityConfig.ymin + humidityConfig.ymax) / 2
            ...
            const updateHumidityLegend = () => {
              ...
              const currentDataName =
                humidityTexturePaths[humidityConfig.currentTextureIndex]
                  ?.name || '数据'
            ...
            try {
              ...
              const updateHumidity = () => {
                if (
                  window.humidityInstance &&
                  window.humidityInstance.primitive &&
                  window.humidityInstance.primitive.appearance &&
                  window.humidityInstance.primitive.appearance.uniforms
                ) {
                  const uniforms =
                    window.humidityInstance.primitive.appearance.uniforms
                  ...
                  uniforms.humidityColorLow = hexToRgbVector(
                    humidityConfig.humidityLowColor
                  )
                  uniforms.humidityColorMid = hexToRgbVector(
                    humidityConfig.humidityMidColor
                  )
                  uniforms.humidityColorHigh = hexToRgbVector(
                    humidityConfig.humidityHighColor
                  )
                  // 剖切参数
                  ...
                  // 颜色过滤参数
                  uniforms.colorFilterEnabled =
                    humidityConfig.colorFilterEnabled
                  uniforms.targetColor = hexToRgbVector(
                    humidityConfig.targetColor
                  )
              ...
              humidityFolder
                .add(humidityConfig, 'steps', 50, 400)
                .step(1)
                .name('采样步数')
              ...
              humidityFolder
                .add(humidityConfig, 'opacityScale', 0.1, 3)
                .name('透明度')
                .onChange(updateHumidity)
              ...
              const switchTexture = async (index) => {
                if (
                  index >= 0 &&
                  index < humidityTexturePaths.length &&
                  window.humidityInstance &&
                  window.humidityInstance.primitive &&
                  window.humidityInstance.primitive.appearance
                ) {
                  ...
                  earth.Resource.createIfNeeded(newPath)
                    ...
                    .then((res) => {
                      ...
                      window.humidityInstance.primitive.appearance.uniforms.cubeTex =
                        cubeTex
                      updateHumidityLegend()
                    })
                    .catch((error) => {
              ...
              const textureSelector = humidityFolder
                .add(humidityConfig, 'currentTextureIndex', textureNames)
                .name('数据切换')
              textureSelector.onChange(async (selectedName) => {
                const selectedIndex = humidityTexturePaths.findIndex(
              ...
              clipXFolder
                .add(humidityConfig, 'clipXEnabled')
                .name('启用')
                .onChange(updateHumidity)
              clipXFolder
                .add(humidityConfig, 'clipXMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateHumidity)
              clipXFolder
                .add(humidityConfig, 'clipXMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateHumidity)
              ...
              clipYFolder
                .add(humidityConfig, 'clipYEnabled')
                .name('启用')
                .onChange(updateHumidity)
              clipYFolder
                .add(humidityConfig, 'clipYMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateHumidity)
              clipYFolder
                .add(humidityConfig, 'clipYMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateHumidity)
              ...
              clipZFolder
                .add(humidityConfig, 'clipZEnabled')
                .name('启用')
                .onChange(updateHumidity)
              clipZFolder
                .add(humidityConfig, 'clipZMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateHumidity)
              clipZFolder
                .add(humidityConfig, 'clipZMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateHumidity)
              ...
              colorFilterFolder
                .add(humidityConfig, 'colorFilterEnabled')
                .name('启用颜色过滤')
                .onChange(updateHumidity)
              colorFilterFolder
                .addColor(humidityConfig, 'targetColor')
                .name('目标颜色')
                .onChange(updateHumidity)
              colorFilterFolder
                .add(humidityConfig, 'colorTolerance', 0.0, 1.0)
                .name('颜色容差')
                .onChange(updateHumidity)
              ...
              playbackFolder
                .add(playbackConfig, 'interval', 0.1, 5)
                .name('切换间隔(秒)')
                ...
                toggle: () => {
                  if (playbackConfig.isPlaying) {
                    ...
                    const tick = () => {
                      ...
                      textureSelector.setValue(
                        humidityTexturePaths[nextIndex].name
                      )
                      textureSelector.updateDisplay()

                      if (playbackConfig.isPlaying) {
                        playbackConfig.timer = setTimeout(
                          tick,
                          playbackConfig.interval * 1000
                        )
                      }
                    }
              ...
              const playbackButton = playbackFolder
                .add(togglePlayback, 'toggle')
                .name('▶ 播放')
          ...
          case 'icetj': //积冰
            ...
            const iceTexturePaths = [
              ...
              {
                name: '03:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.003.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '06:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.006.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '09:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.009.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '12:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.012.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '15:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.015.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '18:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.018.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '21:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.021.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '24:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.024.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '27:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.027.grb_lat_vertical_16x16_blue.png'
              },
              {
                name: '30:00',
                path: '/static/image/texture/ICEpicture_BLUE_new/KTICE2025111000.030.grb_lat_vertical_16x16_blue.png'
              }
            ]
            ...
            const updateIceLegend = () => {
              ...
              const currentDataName =
                iceTexturePaths[iceConfig.currentTextureIndex]?.name || '数据'
              // <div style="display: flex; justify-content: space-between; margin-top: 4px;">
              //   <span>0</span>
              //   <span>5</span>
              //   <span>8</span>
            ...
            try {
              ...
              const updateIce = () => {
                if (
                  window.iceInstance &&
                  window.iceInstance.primitive &&
                  window.iceInstance.primitive.appearance &&
                  window.iceInstance.primitive.appearance.uniforms
                ) {
                  const uniforms =
                    window.iceInstance.primitive.appearance.uniforms
                  ...
                  uniforms.humidityColorLow = hexToRgbVector(
                    iceConfig.humidityLowColor
                  )
                  uniforms.humidityColorMid = hexToRgbVector(
                    iceConfig.humidityMidColor
                  )
                  uniforms.humidityColorHigh = hexToRgbVector(
                    iceConfig.humidityHighColor
                  )
              ...
              iceFolder
                .add(iceConfig, 'steps', 50, 400)
                .step(1)
                .name('采样步数')
                .onChange(updateIce)
              iceFolder
                .add(iceConfig, 'alphaPower', 0.1, 5)
                .name('Alpha幂次')
                .onChange(updateIce)
              iceFolder
                .add(iceConfig, 'minThreshold', 0, 1)
                .name('最小阈值')
                .onChange(updateIce)
              iceFolder
                .add(iceConfig, 'maxThreshold', 0, 1)
                .name('最大阈值')
                .onChange(updateIce)
              iceFolder
                .add(iceConfig, 'opacityScale', 0.1, 3)
                .name('不透明度缩放')
                .onChange(updateIce)
                ...
                if (
                  ...
                  earth.Resource.createIfNeeded(newPath)
                    ...
                    .then((res) => {
                      ...
                      cubeTex.type = 'sampler2D'
                      window.iceInstance.primitive.appearance.uniforms.cubeTex =
                        cubeTex
                      updateIceLegend()
                    })
                    .catch((error) => {
              ...
              const textureSelector = iceFolder
                .add(iceConfig, 'currentTextureIndex', iceNames)
                .name('数据切换')
              textureSelector.onChange(async (selectedName) => {
                const selectedIndex = iceTexturePaths.findIndex(
                  (t) => t.name === selectedName
                )
                if (selectedIndex !== -1) {
                  await switchTexture(selectedIndex)
              ...
              const clipXFolder = clipFolder.addFolder('X轴剖切')
              clipXFolder
                .add(iceConfig, 'clipXEnabled')
                .name('启用')
                .onChange(updateIce)
              clipXFolder
                .add(iceConfig, 'clipXMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateIce)
              clipXFolder
                .add(iceConfig, 'clipXMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateIce)
              ...
              const clipYFolder = clipFolder.addFolder('Y轴剖切')
              clipYFolder
                .add(iceConfig, 'clipYEnabled')
                .name('启用')
                .onChange(updateIce)
              clipYFolder
                .add(iceConfig, 'clipYMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateIce)
              clipYFolder
                .add(iceConfig, 'clipYMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateIce)
              ...
              const clipZFolder = clipFolder.addFolder('Z轴剖切')
              clipZFolder
                .add(iceConfig, 'clipZEnabled')
                .name('启用')
                .onChange(updateIce)
              clipZFolder
                .add(iceConfig, 'clipZMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateIce)
              clipZFolder
                .add(iceConfig, 'clipZMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateIce)
              ...
              colorFilterFolder
                .add(iceConfig, 'colorFilterEnabled')
                .name('启用颜色过滤')
                .onChange(updateIce)
              colorFilterFolder
                .addColor(iceConfig, 'targetColor')
                .name('目标颜色')
                .onChange(updateIce)
              colorFilterFolder
                .add(iceConfig, 'colorTolerance', 0.0, 1.0)
                .name('颜色容差')
                .onChange(updateIce)
            ...
            break
          case 'turbulencetj': //颠簸
            let turbulenceOptions = {}
            turbulenceOptions.name =
              weatherDataConfig.turbulence.texturePaths[1].name
            turbulenceOptions.path =
              weatherDataConfig.turbulence.texturePaths[1].path
            createTurbulence(turbulenceOptions)
            return
            ...
            const turbulenceTexturePaths = [
              {
                name: '00:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0000_lat_vertical_16x16_yellow.png'
              },
              {
                name: '01:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0100_lat_vertical_16x16_yellow.png'
              },
              {
                name: '02:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0200_lat_vertical_16x16_yellow.png'
              },
              {
                name: '03:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0300_lat_vertical_16x16_yellow.png'
              },
              {
                name: '04:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0400_lat_vertical_16x16_yellow.png'
              },
              {
                name: '05:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0500_lat_vertical_16x16_yellow.png'
              },
              {
                name: '06:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0600_lat_vertical_16x16_yellow.png'
              },
              {
                name: '07:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0700_lat_vertical_16x16_yellow.png'
              },
              {
                name: '08:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0800_lat_vertical_16x16_yellow.png'
              },
              {
                name: '09:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_0900_lat_vertical_16x16_yellow.png'
              },
              {
                name: '10:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1000_lat_vertical_16x16_yellow.png'
              },
              {
                name: '11:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1100_lat_vertical_16x16_yellow.png'
              },
              {
                name: '12:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1200_lat_vertical_16x16_yellow.png'
              },
              {
                name: '13:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1300_lat_vertical_16x16_yellow.png'
              },
              {
                name: '14:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1400_lat_vertical_16x16_yellow.png'
              },
              {
                name: '15:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1500_lat_vertical_16x16_yellow.png'
              },
              {
                name: '16:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1600_lat_vertical_16x16_yellow.png'
              },
              {
                name: '17:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1700_lat_vertical_16x16_yellow.png'
              },
              {
                name: '18:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1800_lat_vertical_16x16_yellow.png'
              },
              {
                name: '19:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_1900_lat_vertical_16x16_yellow.png'
              },
              {
                name: '20:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_2000_lat_vertical_16x16_yellow.png'
              },
              {
                name: '21:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_2100_lat_vertical_16x16_yellow.png'
              },
              {
                name: '22:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_2200_lat_vertical_16x16_yellow.png'
              },
              {
                name: '23:00',
                path: '/static/image/texture/TURBpicture_YELLOW_100m_2026-05-16_214755/TI1_result_2300_lat_vertical_16x16_yellow.png'
              }
            ]
            ...
            const turbulencePrimitive =
              turbulenceDC.createTurbulenceTextureAliasOD(turbulenceConfig)
            window.turbulenceInstance = {
              DC: turbulenceDC,
              primitive: turbulencePrimitive,
              config: turbulenceConfig
            }

            const turbulenceCenterLon =
              (turbulenceConfig.xmin + turbulenceConfig.xmax) / 2
            const turbulenceCenterLat =
              (turbulenceConfig.ymin + turbulenceConfig.ymax) / 2
            const turbulenceLonDiff =
              turbulenceConfig.xmax - turbulenceConfig.xmin
            const turbulenceLatDiff =
              turbulenceConfig.ymax - turbulenceConfig.ymin
            const turbulenceMaxDiff = Math.max(
              turbulenceLonDiff,
              turbulenceLatDiff
            )
            ...
            const updateTurbulenceLegend = () => {
              ...
              const currentDataName =
                turbulenceTexturePaths[turbulenceConfig.currentTextureIndex]
                  ?.name || '数据'
              // <div style="display: flex; justify-content: space-between; margin-top: 4px;">
              //   <span>0</span>
              //   <span>5</span>
            ...
            try {
              ...
              const updateTurbulence = () => {
                if (
                  window.turbulenceInstance &&
                  window.turbulenceInstance.primitive &&
                  window.turbulenceInstance.primitive.appearance &&
                  window.turbulenceInstance.primitive.appearance.uniforms
                ) {
                  const uniforms =
                    window.turbulenceInstance.primitive.appearance.uniforms
                  ...
                  uniforms.humidityColorLow = hexToRgbVector(
                    turbulenceConfig.humidityLowColor
                  )
                  uniforms.humidityColorMid = hexToRgbVector(
                    turbulenceConfig.humidityMidColor
                  )
                  uniforms.humidityColorHigh = hexToRgbVector(
                    turbulenceConfig.humidityHighColor
                  )
                  ...
                  uniforms.colorFilterEnabled =
                    turbulenceConfig.colorFilterEnabled
                  uniforms.targetColor = hexToRgbVector(
                    turbulenceConfig.targetColor
                  )
              ...
              turbulenceFolder
                .add(turbulenceConfig, 'steps', 50, 400)
                .step(1)
                .name('采样步数')
                .onChange(updateTurbulence)
              turbulenceFolder
                .add(turbulenceConfig, 'alphaPower', 0.1, 5)
                .name('Alpha幂次')
                .onChange(updateTurbulence)
              turbulenceFolder
                .add(turbulenceConfig, 'minThreshold', 0, 1)
                .name('最小阈值')
                .onChange(updateTurbulence)
              turbulenceFolder
                .add(turbulenceConfig, 'maxThreshold', 0, 1)
                .name('最大阈值')
                .onChange(updateTurbulence)
              turbulenceFolder
                .add(turbulenceConfig, 'opacityScale', 0.1, 3)
                .name('不透明度缩放')
                .onChange(updateTurbulence)
              ...
              const switchTexture = async (index) => {
                if (
                  index >= 0 &&
                  index < turbulenceTexturePaths.length &&
                  window.turbulenceInstance &&
                  window.turbulenceInstance.primitive &&
                  window.turbulenceInstance.primitive.appearance
                ) {
                  ...
                  earth.Resource.createIfNeeded(newPath)
                    ...
                    .then((res) => {
                      ...
                      cubeTex.type = 'sampler2D'
                      window.turbulenceInstance.primitive.appearance.uniforms.cubeTex =
                        cubeTex
                      updateTurbulenceLegend()
                    })
                    .catch((error) => {
              ...
              const turbulenceNames = turbulenceTexturePaths.map(
                (item) => item.name
              )
              const textureSelector = turbulenceFolder
                .add(turbulenceConfig, 'currentTextureIndex', turbulenceNames)
                .name('数据切换')
              textureSelector.onChange(async (selectedName) => {
                const selectedIndex = turbulenceTexturePaths.findIndex(
                  (t) => t.name === selectedName
                )
                if (selectedIndex !== -1) {
                  await switchTexture(selectedIndex)
                ...
                toggle: () => {
                  if (playbackConfig.isPlaying) {
                    ...
                    const tick = () => {
                      ...
                      switchTexture(nextIndex)
                      textureSelector.setValue(
                        turbulenceTexturePaths[nextIndex].name
                      )
                      textureSelector.updateDisplay()
              ...
              const clipXFolder = clipFolder.addFolder('X轴剖切')
              clipXFolder
                .add(turbulenceConfig, 'clipXEnabled')
                .name('启用')
                .onChange(updateTurbulence)
              clipXFolder
                .add(turbulenceConfig, 'clipXMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateTurbulence)
              clipXFolder
                .add(turbulenceConfig, 'clipXMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateTurbulence)
              ...
              const clipYFolder = clipFolder.addFolder('Y轴剖切')
              clipYFolder
                .add(turbulenceConfig, 'clipYEnabled')
                .name('启用')
                .onChange(updateTurbulence)
              clipYFolder
                .add(turbulenceConfig, 'clipYMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateTurbulence)
              clipYFolder
                .add(turbulenceConfig, 'clipYMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateTurbulence)
              ...
              const clipZFolder = clipFolder.addFolder('Z轴剖切')
              clipZFolder
                .add(turbulenceConfig, 'clipZEnabled')
                .name('启用')
                .onChange(updateTurbulence)
              clipZFolder
                .add(turbulenceConfig, 'clipZMin', 0.0, 1.0)
                .name('最小位置')
                .onChange(updateTurbulence)
              clipZFolder
                .add(turbulenceConfig, 'clipZMax', 0.0, 1.0)
                .name('最大位置')
                .onChange(updateTurbulence)
              ...
              colorFilterFolder
                .add(turbulenceConfig, 'colorFilterEnabled')
                .name('启用颜色过滤')
                .onChange(updateTurbulence)
              colorFilterFolder
                .addColor(turbulenceConfig, 'targetColor')
                .name('目标颜色')
                .onChange(updateTurbulence)
              colorFilterFolder
                .add(turbulenceConfig, 'colorTolerance', 0.0, 1.0)
                .name('颜色容差')
                .onChange(updateTurbulence)
          ...
          case 'humidity2': //湿度区域2
            this.dataController.addHumidity(
              '/static/config/json/parsed_humidity_r4c4.json',
              '湿度2'
            )
            break
          case 'humidity3': //湿度区域3
            this.dataController.addHumidity(
              '/static/config/json/parsed_humidity_r3c4.json',
              '湿度3'
            )
            break
          case 'cloud': //云层1
            let cloudOptions = {}
            // cloudOptions.name = cloudList[1].name
            // cloudOptions.path = cloudList[1].path
            // createCloud(cloudOptions)
            cloud_interval = setInterval(() => {
              let isHave = false
              let AT = store.state.AFSIMModule.ATValue
              if (AT >= 1 && AT < 180) {
                cloudOptions.name = weatherDataConfig.cloud.texturePaths[2].name
                cloudOptions.path = weatherDataConfig.cloud.texturePaths[2].path
              } else if (AT > 180) {
                cloudOptions.name =
                  weatherDataConfig.cloud.texturePaths[23].name
                cloudOptions.path =
                  weatherDataConfig.cloud.texturePaths[23].path
              }
            ...
            break
          case 'cloud2': //云层2
            this.dataController.addCloud(
              '/static/config/json/parsed_cloud_r4c4.json',
              '云层2'
            )
            break
          case 'cloud3': //云层3
            this.dataController.addCloud(
              '/static/config/json/parsed_cloud_r3c4.json',
              '云层3'
            )
            break
        ...
        switch (val.code) {
          ...
          case 'windtj': //风场可调节
            ...
            if (wind3dInstance) {
              if (
                typeof wind3dInstance.isDestroyed === 'function' &&
                !wind3dInstance.isDestroyed()
              ) {
          ...
          case 'ARMultiPoints': //地表影像
            ...
            break
          case 'humiditytj': //湿度可调节
            removeHumidity(window.EarthViewer)
            return
            ...
            if (humidityInstance && humidityInstance.primitive) {
              try {
                window.EarthViewer.scene.primitives.remove(
                  humidityInstance.primitive
                )
              } catch (e) {
          ...
          case 'icetj': //积冰
            ...
            break
          case 'turbulencetj': //颠簸
            removeTurbulence(window.EarthViewer)
            return
            ...
            const turbulenceLegend =
              document.getElementById('turbulence-legend')
            if (turbulenceLegend) {
  ...
  loadSmlFile() {
    ...
    this.loadSml('bh (1).sml', callback)
  }
