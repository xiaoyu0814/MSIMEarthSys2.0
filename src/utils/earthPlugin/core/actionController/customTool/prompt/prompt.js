...
class Prompt {
  ...
  constructor(viewer, opt) {
    ...
    let anchorHtml = ``
    let closeHtml = ``
    let background = this.opt.style.background
    let color = this.opt.style.color
    let fieldColor = this.opt.style.fieldColor || 'yellow' // 字段颜色
    let contentColor = this.opt.style.contentColor || 'chartreuse' // 内容颜色
    let boxShadow = this.opt.style.boxShadow || '0 0 25px #1092d5;' //边框阴影
    if (this.opt.anchor) {
      anchorHtml += `
    ...
    if (this.opt.content.length > 0) {
      this.opt.content.forEach((element) => {
        contentText += ` <div style="display: flex;"><div style="color:${fieldColor}">${element.type}：</div>
    ...
    const promptConenet = `
                <!-- 文本内容 -->
                <div class="prompt-content-container" style="background:${background} !important;color:${color} !important;border:none !important;box-shadow:${boxShadow} !important">
  ...
  bindRender() {
    ...
    this.rendHandler = this.viewer.scene.postRender.addEventListener(
      function () {
        ...
        if (that.position instanceof window.MSIMEarth.Cartesian3) {
          let px = window.MSIMEarth.SceneTransforms.worldToWindowCoordinates(
            that.viewer.scene,
            that.position
          )
          if (!px) return
   ...
   */
  update(px, html) {
    if (px instanceof window.MSIMEarth.Cartesian3) {
      this.position = px.clone()
      px = window.MSIMEarth.SceneTransforms.worldToWindowCoordinates(
        this.viewer.scene,
        px
      )
    }
  ...
  isInView() {
    ...
    let px = null
    if (this.position instanceof window.MSIMEarth.Cartesian2) {
      px = this.position
    } else {
      px = window.MSIMEarth.SceneTransforms.worldToWindowCoordinates(
        this.viewer.scene,
  ...
  transPosition(p) {
    let position
    if (Array.isArray(p)) {
      const posi = window.MSIMEarth.Cartesian3.fromDegrees(
        p[0],
        p[1],
        p[2] || 0
      )
      position = posi.clone()
    } else if (p instanceof window.MSIMEarth.Cartesian3) {
      position = p.clone()
    } else {
      // 像素类型
      position = p
