...
import { airplaneSensorON, airplaneSensorONFP, airplaneSensorOFF } from './stateControlMethods'
...
export default function () {
  ...
  const handleState = (json) => {
    switch (json.Type) {
      case 'SU':
        //可见光卫星开机或关机 当前也包括wz系列和各种制导导弹但是没还没和AFSIM对好镜头
        if (satellitePlateformArr.indexOf(json.Data.PName) > -1) {
          // 如果json.Data.PName包含high，说明是高轨道卫星，设置为high轨道
          let orbitType
          if (json.Data.PName.indexOf('high') > -1) {
            orbitType = 'high'
          } else if (json.Data.PName.indexOf('medium') > -1) {
            orbitType = 'medium'
          } else if (json.Data.PName.indexOf('low') > -1) {
            orbitType = 'low'
          } else {
            orbitType = 'basic'
          }
