 ...
 * @Version: 1.0
 * @Author: ZX Li
 * @Date: 2025-09-05 15:22:25
 * @LastEditors: ZX Li
 * @LastEditTime: 2025-09-05 16:13:03
 */
export default function () {
  // 创建激光跟踪器实体
  const createLaser_tracker = (params, volumesData) => {
    ...
    volumesData.Modes.forEach((Mode) => {
      let id = entityTarget.id + '_laser_tracker'
  ...
  // 实体被干扰
  const laser_trackerJam = (params, volumesData) => {
    let entityTarget = window.EarthPlugn.entity._GetCZMLEntity(
      params.name,
      'MSIMEarthCZMLProcessContainer'
    )
