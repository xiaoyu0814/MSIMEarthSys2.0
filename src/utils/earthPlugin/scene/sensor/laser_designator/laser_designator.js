 ...
 * @Version: 1.0
 * @Author: ZX Li
 * @Date: 2025-09-05 15:02:29
 * @LastEditors: ZX Li
 * @LastEditTime: 2025-09-05 15:05:14
 */
export default function () {
  // 初始化激光设计器类型sensor
  const createLaserDesignator = (params, volumesData) => {
    ...
    volumesData.Modes.forEach((Mode) => {
      let id = entityTarget.id + '_laser_designator'
  ...
  // 实体被干扰
  const laser_designatorJam = (params, volumesData) => {
    let entityTarget = window.EarthPlugn.entity._GetCZMLEntity(
      params.name,
      'MSIMEarthCZMLProcessContainer'
    )
  ...
  const removeLaserDesignator = (id) => {
    window.EarthViewer.entities.removeById(id)
  }

  return {
    createLaserDesignator,
    laser_designatorJam,
    removeLaserDesignator
  }
}
