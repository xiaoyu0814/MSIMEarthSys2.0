<template>
  <div id="taskManagement">
    <div class="search_create">
      <el-select v-model="vueData.searchCondition">
        <el-option
          v-for="item in vueData.searchConditionList"
          :key="item.type"
          :label="item.name"
          :value="item.type"
        />
      ...
      <div class="time_condition">
        <el-date-picker
          v-if="
            vueData.searchCondition == 'createTime' ||
            vueData.searchCondition == 'changeTime'
          "
          v-model="vueData.timeSection"
          type="datetimerange"
          range-separator="至"
      ...
      >
        ...
        <CaretBottom
          ...
          :class="vueData.sortType == 'down' ? 'selectedStyle' : ''"
          @click="setSortType('down')"
        />
        <!-- 升序 -->
        <CaretTop
          title="升序"
          :class="vueData.sortType == 'up' ? 'selectedStyle' : ''"
          style="width: 30px; height: 30px; color: #fff"
          @click="setSortType('up')"
        />
      </div>
      <el-icon><icon-picture /></el-icon>
      <el-input
        v-show="vueData.searchCondition == 'name'"
...
const vueData = reactive({
  ...
  searchConditionList: [
    //搜索条件
    { name: '名称', type: 'name' },
    { name: '生成时间', type: 'createTime' },
    { name: '修改时间', type: 'changeTime' }
  ],
  searchCondition: 'name',
  timeSection: ['2024-05-22 12:00:00', '2024-12-22 12:00:00'],
  sortType: ''
})
...
let _getTaskList = () => {
  ...
  //   pageSize: vueData.pageSize
  // }
  // if (vueData.searchCondition == 'name') {
  //   params['name'] = vueData.search
  // } else if (vueData.searchCondition == 'createTime') {
  //   params['createStartTime'] = vueData.timeSection[0]
  //   params['createEndTime'] = vueData.timeSection[1]
  // } else if (vueData.searchCondition == 'changeTime') {
  //   params['updatesStartTime'] = vueData.timeSection[0]
  //   params['updatesEndTime'] = vueData.timeSection[1]
  // }
  // if (vueData.sortType == 'down') {
  //   //降
  //   params['sortType'] = 0 //倒序
  // } else if (vueData.sortType == 'up') {
  //   //升
  //   params['sortType'] = 1 //正序
  // }

  // getTaskListByConditions(params, vueData.uploadHeaders).then((res) => {
  //   //console.log(res)
  //   if (res.code == 200) {
  //     vueData.taskList = res.data.records
  //     vueData.total = res.data.total
