...
watch(
  () => props.currentEidtData,
  (newVal, oldVal) => {
    console.log(newVal)
    if (newVal) {
      vueData.currentId = newVal[0].id
      vueData.troopsForm.type = newVal[0].object.entityID
      for (let i = 0; i < vueData.typeOption2.length; i++) {
        if (vueData.typeOption2[i].recordType == newVal[0].operationType) {
          vueData.troopsForm2.type = vueData.typeOption2[i].recordTypeName
        }
      }
      vueData.troopsForm.planName = newVal[0].planName
    }
  },
  { deep: true }
)
...
const save = (val) => {
  ...
  if (val == '立即执行') {
    ...
    editConductPlan(runSeconds, time)
  } else if (val == '保存') {
    if (vueData.time == '') {
    ...
    editConductPlan(runSeconds, time)
    emitter.emit('closeEditPlan', false)
  }
}

// 修改导调计划
const editConductPlan = (val1, val2) => {
  let params = {
    camp: '',
    id: vueData.currentId,
    object: {
    ...
    operationType: vueData.troopsForm2.type,
    planName: vueData.troopsForm.planName,
    recordType: '',
    time: val1,
    planTime: val2
  }
  updateListById(params).then((res) => {
    console.log(res)
    if (res.code == 200) {
