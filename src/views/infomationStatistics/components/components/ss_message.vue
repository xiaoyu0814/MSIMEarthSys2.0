  ...
  <div id="ss_message">
    <div class="collapse-interaction" id="log_con">
      <div class="single-log" v-for="item in state.logList" :key="item">
        ...
        <div class="log-side">
          <div class="log-item">
            <span class="time" v-if="props.showSceneTime">
              {{ item.sendTime }}
            </span>
            <!-- <span>{{ item.msg }}</span> -->
            <!-- 判断 如果是合并的多条数据 -->
    ...
    if (state.timeOver) {
      ...
      setTimeout(() => {
        ...
        let czmlEventSourceData = {
          ...
          msg: '',
          camp: '',
          sendTime: props.startTime
          // sendTime: getTime(
          //   new Date(startTime + obj.time),
          //   'yyyy-MM-dd HH:mm:ss'
          // )
        }
        // 新日志消息对接
...
// setInterval(() => {
//   let newValue = store.state.sceneModule.czmlEventSourceData
//   let obj = newValue
...
//     msg: '',
//     camp: '',
//     sendTime: getTime(new Date(startTime + obj.time), 'yyyy-MM-dd HH:mm:ss')
//   }
//   // 新日志消息对接
//   czmlEventSourceData.msg = newValue.message
//   if (newValue.objectiveSide) {
