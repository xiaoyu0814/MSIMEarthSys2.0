      ...
      <div class="taskItem_box">
        ...
        >
          <div class="item_header" style="position: relative">
            ...
            >
              ...
              <!-- <el-tooltip
                ...
                placement="top-start"
              >
                <el-icon class="list-icon" @click.stop="openSamplePanel(item)"
                  ><Collection
                /></el-icon>
              </el-tooltip> -->
              <!-- <el-tooltip
          ...
          <ul class="item_content">
            ...
            >
              运行状态:{{ item.simRunStatus == 2 ? '运行中' : '未运行' }}
            </li>
 ...
 * 配置实验过程中汇报的内容
 */
const _configExperimentReportContent = (row) => {
  sceneInfoConfig(row)
...
const _getList = () => {
  ...
  if (curUrl.split('?').length > 1) {
    ...
    creategetPage(params).then((res) => {
      ...
      console.log('实验列表2', vueData.taskList)
